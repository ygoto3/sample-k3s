...
impl DatabaseSession {
    ...
    pub async fn new(db_url: String, db_user: String, db_password: String) -> Result<Self, DbErr> {
        let db_connection = Database::connect(format!("postgresql://{}:{}@{}", db_user, db_password, db_url)).await?;
...
pub async fn init_session() -> Result<(), String> {
    ...
    for env_url in env_urls.iter() {
        let db_url = env::var(env_url).unwrap_or_else(|_| "127.0.0.1:5432/db".to_string());
        let db_user = env::var("DATABASE_USER").unwrap_or_else(|_| "postgres".to_string());
        let db_password = env::var("DATABASE_PASSWORD").unwrap_or_else(|_| "postgresqldevpassword".to_string());
